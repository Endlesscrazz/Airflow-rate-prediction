...
(Version 8: True real-time simulation with dynamic/adaptive cropping)
"""
import os
import sys
import argparse
...
def calculate_temporal_trend_map_theilsen_cpu(frames):
    ...
    print("\n--- (Slow Path) Using CPU Parallel Theil-Sen (Robust) ---")
...
# <--- MODIFIED: OLS Calculator now uses a mask for dynamic cropping ---
class OnlineOLSCalculator:
    ...
    def add_frame(self, frame, mask):
        """Only update the sums within the valid area defined by the mask."""
        t = self.n
        self.sum_x += t
        # Apply mask to only update valid pixels
    ...
    def calculate_slope(self):
        ...
        # Avoid division by zero and only calculate for pixels that have received data
...
class SlowPathAnalyzer(threading.Thread):
    def __init__(self, num_leaks, analysis_interval_frames, regressor, use_gpu, compare_mode):
        ...
        self.regressor = regressor; self.use_gpu = use_gpu; self.compare_mode = compare_mode
        self.results = {}; self._stop_event = threading.Event()

    def add_frame(self, frame): self.frame_buffer.append(frame)

    def calculate_slope_map_huber_gpu(self, frames_np):
        # (This function remains the same, but it's now called from _run_analysis)
        print(f"\n--- (Slow Path) Using GPU Accelerated Huber (Robust, Fast) ---")
        ...
        slope = torch.sum((t_gpu - t_mean) * (frames_gpu - frames_mean), dim=2) / torch.sum((t_gpu - t_mean)**2, dim=2)
        ...
        for _ in range(5):
            ...
            w_sum = torch.sum(weights, dim=2); wx_sum = torch.sum(weights * t_gpu, dim=2)
            wy_sum = torch.sum(weights * frames_gpu, dim=2); wxy_sum = torch.sum(weights * t_gpu * frames_gpu, dim=2)
    ...
    def _run_analysis(self, frames_np, regressor_type):
        start_time = time.perf_counter(); device_used = "CPU (Parallel)"
        if regressor_type == 'huber' and self.use_gpu and TORCH_AVAILABLE and torch.cuda.is_available():
    ...
    def run(self):
        ...
        # <--- MODIFIED: The slow path now crops the final buffer before analysis ---
        frames_to_process = list(self.frame_buffer)
        all_frames_np = np.stack([f['frame'] for f in frames_to_process], axis=2)
        all_transforms = [f['transform'] for f in frames_to_process]
        
        dx_vals = [t[0] for t in all_transforms]; dy_vals = [t[1] for t in all_transforms]
        y_start = math.ceil(max(dy_vals)); y_end = all_frames_np.shape[0] + math.floor(min(dy_vals))
        x_start = math.ceil(max(dx_vals)); x_end = all_frames_np.shape[1] + math.floor(min(dx_vals))
...
# --- Visualization & Logging Functions (Unchanged) ---
...
def write_log_file(args, timings, slow_path_thread):
    ...
    with open(log_path, 'w') as f:
        ...
        for arg, value in sorted(vars(args).items()): f.write(f"{arg:<25}: {value}\n")
        f.write("\n--- Performance Metrics ---\n")
        ...
        for regressor, result in slow_path_thread.results.items():
            ...
            f.write(f"Safe Crop Window (HxW): {slow_path_thread.crop_dims[0]}x{slow_path_thread.crop_dims[1]}\n")
            if result['leaks']:
                f.write(f"Found {len(result['leaks'])} leak(s):\n")
                for i, leak in enumerate(result['leaks']):
                    f.write(f"  - Leak #{i+1}: Coords={leak['coords']}, Score={leak['score']:.6f}\n")
            else: f.write("  - No leaks were detected.\n")
...
def main():
    ...
    parser.add_argument("--shift1_x", type=float, default=-5.0); parser.add_argument("--shift1_y", type=float, default=0.0)
    parser.add_argument("--shift2_frame", type=int, default=150)
    parser.add_argument("--shift2_x", type=float, default=10.0); parser.add_argument("--shift2_y", type=float, default=0.0)
    ...
    # <--- MODIFIED: Initialize full-size OLS calculator and transform history ---
    ols_calculator = OnlineOLSCalculator(H, W)
    transform_history = deque(maxlen=T) # Store transforms for the entire window
    slow_path_thread = SlowPathAnalyzer(args.num_leaks, T, args.regressor, args.gpu, args.compare)
    ...
    for i in tqdm(range(T), desc="Simulating real-time feed", file=sys.stdout):
        ...
        # <--- MODIFIED: Calculate dynamic crop based on transform history ---
        dx_vals = [t[0] for t in transform_history]; dy_vals = [t[1] for t in transform_history]
        y_start = math.ceil(max(dy_vals)); y_end = H + math.floor(min(dy_vals))
        x_start = math.ceil(max(dx_vals)); x_end = W + math.floor(min(dx_vals))

        # Create a mask for the valid region and update OLS
        mask = np.zeros((H, W), dtype=bool)
        mask[y_start:y_end, x_start:x_end] = True
        ols_calculator.add_frame(stabilized_frame, mask)
        
        # Feed full frame to slow path; it will crop later
        slow_path_thread.add_frame({'frame': stabilized_frame, 'transform': (dx, dy)})
        ...
        if args.live_preview:
            # Crop the activity map for display only
            cropped_map_for_display = display_activity_map[y_start:y_end, x_start:x_end]
            activity_display = np.zeros((cropped_map_for_display.shape[0], cropped_map_for_display.shape[1], 3), dtype=np.uint8)
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'): slow_path_thread.stop(); break
    ...
    timings = {'total': total_end_time - total_start_time, 'fast_path': main_loop_end_time - main_loop_start_time}
    ...
    create_comparison_gif(shaky_frames, "1. Shaky", stabilized_frames_np, "2. Stabilized", gif_path, vmin, vmax)
    ...
    for regressor, result in slow_path_thread.results.items():
        final_map_path = os.path.join(args.output_dir, f"{base_filename}_{regressor}_final_leak_map.png")
        save_final_leak_map(result['score_map'], result['leaks'], final_map_path)
