...
def asymmetric_loss(preds, targets, over_prediction_penalty=2.0):
    ...
    Args:
        ...
        over_prediction_penalty (float): A multiplier for the loss on samples
                                         where pred > target. Must be >= 1.0.
    """
    base_loss_fn = nn.SmoothL1Loss()
    
    # Create masks to separate over-predictions from under-predictions
    over_mask = preds > targets
    under_mask = ~over_mask

    # Calculate the loss for each case separately
    # The .sum() is used to handle cases where a mask might be all False
    ...
    # Apply the penalty and combine the losses
    # We weight them by the number of elements in each part to keep the total loss scaled correctly
    ...
    # The final loss is a weighted average of the two components
...
def train_one_epoch(model, dataloader, criterion, optimizer, device, scaler):
    ...
    for seq, delta_t, targets_scaled in dataloader:
        ...
        # Clip gradients (same as you had before)
        ...
        # .detach() is important here as outputs_scaled is part of the computation graph
...
def evaluate(model, dataloader, device):
    ...
    # ... (metric calculations are unchanged) ...
...
def main():
    ...
    print(
        f"--- Starting V2 Model Training for Experiment: {cfg.EXPERIMENT_NAME} ---")
    print(f"  - Using data split from random seed: {cfg.RANDOM_STATE}")
    print(
        f"Normalizing dataset '{cfg.EXPERIMENT_NAME}' with max flow rate: {cfg.MAX_FLOW_RATES.get(cfg.EXPERIMENT_NAME, 5.0)}")
    ...
    print(
        f"Loaded {len(train_df_orig)} training and {len(val_df_orig)} validation samples.")
    ...
    train_dataset = CroppedSequenceDataset(
        train_df, DATASET_DIR, transform=train_transform)
    val_dataset = CroppedSequenceDataset(
        val_df, DATASET_DIR, transform=val_transform)
    ...
    print(
        f"Best model (Val MAE: {best_val_mae:.4f}) saved to: {model_save_path}")
    ...
    log_experiment_details(
        log_filepath, "Final Model Training Parameters", final_training_params)
