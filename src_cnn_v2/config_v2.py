...
EXPERIMENT_VERSION = "iter-6-cs15-instance-norm-rs42"
...
# Full path to the generated dataset
DATASET_DIR = os.path.join(OUTPUT_DIR, DATASET_SUBDIR)
TRAIN_METADATA_PATH = os.path.join(
    DATASET_DIR, f"train_metadata_seed{RANDOM_STATE}.csv")
VAL_METADATA_PATH = os.path.join(
    DATASET_DIR, f"val_metadata_seed{RANDOM_STATE}.csv")
TEST_METADATA_PATH = os.path.join(
    DATASET_DIR, f"test_metadata_seed{RANDOM_STATE}.csv")
...
V2_DATASET_PARAMS = {
    ...
    "ENABLE_GEOMETRIC_AUGMENTATION": False,
    "AUGMENTATION_PARAMS": {
        "NOISE_LEVEL": 0.05,
        "ROTATION_DEGREES": 10,
...
MAX_FLOW_RATES = {
    "gypsum_all_dataset_v2": 1.28246,
    "gypsum_10_hole_dataset_v2": 4.6,
    "brickcladding_all_dataset_v2": 1.56860,
    "hardyboard_all_dataset_v2": 1.58645,
}

MAX_FLOW_RATE = MAX_FLOW_RATES.get(
    EXPERIMENT_NAME, 5.0)  # Defaults to 5.0 if not found
...
# GYPSUM ALL
# INITIAL_PARAMS = { tuned on gypsum all dataset
...
# }
# BRICKCLADDING ALL
# INITIAL_PARAMS = { uned on brickcladding all dataset
...
# }
# HARDYBOARD ALL
INITIAL_PARAMS = {  #tuned on hardyboadd all dataset
    'lr': 0.001,
    'weight_decay': 0.0001,
    'dropout_rate': 0.3,
    'lstm_hidden_size': 256,
...
DATASET_CONFIGS = {
    ...
    # OLD
    "hardyboard_0313": {"material": "hardyboard", "dataset_subfolder": "Fluke_HardyBoard_03132025"},
}
