...
def main():
    ...
    for config_key, config in DATASET_CONFIGS.items():
        ...
        material, structure_type, gt_file, session = config['material'], config[
            'structure_type'], config['gt_file'], config['session']
        ...
        if structure_type == 'multi_hole_csv':
            try:
                ...
                print(
                    f"  - Successfully loaded multi-hole CSV ground truth from: {gt_file}")
            except Exception as e:
                print(
                    f"  - ERROR: Could not read multi-hole CSV GT file '{gt_path}'. Error: {e}. Skipping.")
                continue

            processed_videos_count = 0
            video_files = [f for f in os.listdir(folder_path) if f.endswith(
            ...
            for filename in video_files:
                ...
                try:
                    ...
                    print(
                        f"  - WARNING: Pressure {pressure} from filename '{filename}' not found in GT file. Skipping.")
                    continue

                base_record = {'material': material, 'delta_T': delta_T,
                               'voltage': np.nan, 'pressure_Pa': pressure, 'session': session}
            ...
            print(
                f"  - Processed {processed_videos_count} videos from 'multi_hole_csv' structure, generating up to {processed_videos_count * 8} samples.")
            continue  # Move to the next dataset in the config loop

        try:
            if gt_file.endswith('.xlsx'):
                gt_df = pd.read_excel(gt_path, header=None, skiprows=1, names=[
                                      'V_str', 'Pa', 'rate_single'])
                gt_df['V'] = gt_df['V_str'].apply(lambda x: float(
                    re.search(r'(\d+(\.\d+)?)', str(x)).group(1)))
            else:
                if is_two_holes:
                    gt_df = pd.read_csv(
                        gt_path, sep='\s+', header=None, skiprows=1, names=['V', 'Pa', 'rate_1', 'rate_2'])
                else:
                    gt_df = pd.read_csv(
                        gt_path, sep='\s+', header=None, skiprows=1, names=['V', 'Pa', 'rate_single'])

            gt_df['V'] = gt_df['V'].astype(float)
            print(f"  - Successfully loaded ground truth from: {gt_file}")
        except Exception as e:
            print(
                f"  - ERROR: Could not read GT file '{gt_path}'. Error: {e}. Skipping.")
            continue
        ...
        if structure_type == 'old':
            ...
            for subfolder_name in os.listdir(folder_path):
                ...
                if os.path.isdir(subfolder_path):
                    ...
                    video_files = [f for f in os.listdir(subfolder_path) if f.endswith(
                        '.mat') and not f.startswith('._')]
                    for filename in video_files:
                        all_records.append({
                        ...
                        processed_videos_count += 1
            print(
                f"  - Processed {processed_videos_count} videos from 'old' structure.")

        else: # New structure
            # --- FIX: This block is now corrected to handle nested folders ---
            processed_videos_count = 0
            ...
            for subfolder_path, _, files in os.walk(folder_path):
                ...
                if not video_files:
                    continue # Skip empty directories or directories without .mat files

                for filename in video_files:
                    video_id = filename.replace('.mat', '')
                    
                    # Determine voltage from filename, fallback to folder name if needed
                    ...
                    if is_two_holes:
                        # Use simple numeric IDs to match the output of find_leaking_holes.py
    ...
    if all_records:
        ...
        final_df = final_df.sort_values(
            by=['material', 'session', 'voltage', 'delta_T']).reset_index(drop=True)
