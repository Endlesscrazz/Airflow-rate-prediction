# --- START OF MODIFIED feature_engineering.py ---
...
import numpy as np
# import cv2 # Not strictly needed for current features
import os
import scipy.io
import config # Assuming config.py exists for MAT_FRAMES_KEY lookup
...
def calculate_hotspot_features(
    ...
    if N_total < 2: print("Warning: Need >= 2 total frames."); return features
    ...
    # ... (keep initial feature calculation logic exactly same as before) ...
    actual_focus_frames = min(focus_duration_frames, N_total)
    if actual_focus_frames >= 2:
        frames_focus = frames[:, :, :actual_focus_frames]; time_vector_focus = (np.arange(actual_focus_frames) / fps).astype(np.float64)
        masked_pixels_ts_focus = frames_focus[hotspot_mask, :]; finite_pixel_mask_rows_focus = np.all(np.isfinite(masked_pixels_ts_focus), axis=1)
        ...
        if num_valid_pixels_focus > 0:
            ...
            hotspot_avg_temp_series_focus = np.nanmean(valid_masked_pixels_ts_focus, axis=0); valid_avg_temps_mask_focus = np.isfinite(hotspot_avg_temp_series_focus)
            if np.sum(valid_avg_temps_mask_focus) >= 2:
                valid_avg_times_f = time_vector_focus[valid_avg_temps_mask_focus]; valid_avg_temps_f = hotspot_avg_temp_series_focus[valid_avg_temps_mask_focus]
                try: slope, _, _, _, _ = linregress(valid_avg_times_f, valid_avg_temps_f); features['hotspot_avg_temp_change_rate_initial'] = slope if np.isfinite(slope) else np.nan
                except ValueError: pass
                features['hotspot_avg_temp_change_magnitude_initial'] = valid_avg_temps_f[-1] - valid_avg_temps_f[0]
            peak_pixel_temp_list_focus = []; peak_func = np.nanmin if envir_para < 0 else np.nanmax
            for t in range(actual_focus_frames): pixels_in_mask_t_f = valid_masked_pixels_ts_focus[:, t]; peak_pixel_temp_list_focus.append(peak_func(pixels_in_mask_t_f) if pixels_in_mask_t_f.size > 0 else np.nan)
            peak_pixel_temp_series_focus = np.array(peak_pixel_temp_list_focus); valid_peak_temps_mask_focus = np.isfinite(peak_pixel_temp_series_focus)
            if np.sum(valid_peak_temps_mask_focus) >= 2:
                valid_peak_times_f = time_vector_focus[valid_peak_temps_mask_focus]; valid_peak_temps_f = peak_pixel_temp_series_focus[valid_peak_temps_mask_focus]
                try: slope_peak, _, _, _, _ = linregress(valid_peak_times_f, valid_peak_temps_f); features['peak_pixel_temp_change_rate_initial'] = slope_peak if np.isfinite(slope_peak) else np.nan
                except ValueError: pass
                features['peak_pixel_temp_change_magnitude_initial'] = valid_peak_temps_f[-1] - valid_peak_temps_f[0]
            try:
                 with warnings.catch_warnings(): warnings.simplefilter("ignore", category=RuntimeWarning)
                 if np.sum(valid_avg_temps_mask_focus) > 0: features['temp_mean_avg_initial'] = np.nanmean(hotspot_avg_temp_series_focus[valid_avg_temps_mask_focus])
                 std_temps_per_frame_focus = np.nanstd(valid_masked_pixels_ts_focus, axis=0); valid_std_mask_focus = np.isfinite(std_temps_per_frame_focus)
                 if np.any(valid_std_mask_focus): features['temp_std_avg_initial'] = np.nanmean(std_temps_per_frame_focus[valid_std_mask_focus])
                 if valid_masked_pixels_ts_focus.size > 0: features['temp_min_overall_initial'] = np.nanmin(valid_masked_pixels_ts_focus); features['temp_max_overall_initial'] = np.nanmax(valid_masked_pixels_ts_focus)
            except Exception as e: print(f"Warn: Init dist features failed: {e}")

    # --- Calculate Difference Frames & Full Duration Features ---
    try:
        initial_frame = frames[:, :, 0:1]
        # ... (keep check for initial frame validity) ...
        if not np.all(np.isfinite(initial_frame[hotspot_mask,:])): print("Warning: Non-finite values in initial frame within mask.")

        frames_diff = frames - initial_frame

        masked_pixels_diff_ts = frames_diff[hotspot_mask, :]
        finite_pixel_mask_rows_full = np.all(np.isfinite(masked_pixels_diff_ts), axis=1)
        ...
        if num_valid_pixels_full > 0:
            ...
            # ... (keep logic exactly the same for these mean/std deltaT features) ...
            with warnings.catch_warnings(): warnings.simplefilter("ignore", category=RuntimeWarning)
            hotspot_avg_temp_diff_series = np.nanmean(valid_masked_pixels_diff_ts, axis=0); hotspot_std_temp_diff_series = np.nanstd(valid_masked_pixels_diff_ts, axis=0)
            valid_avg_diff_mask = np.isfinite(hotspot_avg_temp_diff_series); valid_std_diff_mask = np.isfinite(hotspot_std_temp_diff_series)
            stabilization_start_frame = int(N_total * 0.75)
            if np.sum(valid_avg_diff_mask) > 0:
                valid_avg_diff_vals = hotspot_avg_temp_diff_series[valid_avg_diff_mask]; features['overall_mean_deltaT'] = np.mean(valid_avg_diff_vals); features['max_abs_mean_deltaT'] = np.max(np.abs(valid_avg_diff_vals))
                if stabilization_start_frame < N_total: stabilized_mask = valid_avg_diff_mask[stabilization_start_frame:];  features['stabilized_mean_deltaT'] = np.mean(hotspot_avg_temp_diff_series[stabilization_start_frame:][stabilized_mask]) if np.sum(stabilized_mask) > 0 else np.nan
            if np.sum(valid_std_diff_mask) > 0:
                 features['overall_std_deltaT'] = np.mean(hotspot_std_temp_diff_series[valid_std_diff_mask])
                 if stabilization_start_frame < N_total: stabilized_mask_std = valid_std_diff_mask[stabilization_start_frame:]; features['stabilized_std_deltaT'] = np.mean(hotspot_std_temp_diff_series[stabilization_start_frame:][stabilized_mask_std]) if np.sum(stabilized_mask_std) > 0 else np.nan
            ...
            if threshold_abs_change is not None and np.isfinite(threshold_abs_change) and threshold_abs_change >= 0:
                 ...
                 for t in range(N_total):
                      valid_pixels_diff_t = valid_masked_pixels_diff_ts[:, t] # Already filtered for NaNs across time
                      if valid_pixels_diff_t.size > 0:
                           # Use the passed fixed absolute threshold
                           area_significant_change_series[t] = np.sum(np.abs(valid_pixels_diff_t) > threshold_abs_change)
                 ...
                 if np.sum(valid_area_mask) > 0:
                      ...
                      features['mean_area_significant_change'] = np.mean(valid_area_vals)
                      features['max_area_significant_change'] = np.max(valid_area_vals)
                      # Stabilized Area
                      if stabilization_start_frame < N_total:
                           ...
                           if np.sum(stabilized_mask_area) > 0:
                                features['stabilized_area_significant_change'] = np.mean(area_significant_change_series[stabilization_start_frame:][stabilized_mask_area])
            else:
                 print(f"Skipping area features due to invalid fixed threshold: {threshold_abs_change}")
        ...
        else: print("Warning: No valid pixels over full duration for difference features.")

    except Exception as e:
        print(f"Error calculating difference/stabilized/area features: {e}")
        traceback.print_exc()
        # ... (keep reset logic for features on error) ...
        diff_keys = ['stabilized_mean_deltaT', 'overall_mean_deltaT', 'max_abs_mean_deltaT', 'stabilized_std_deltaT', 'overall_std_deltaT', 'mean_area_significant_change', 'stabilized_area_significant_change', 'max_area_significant_change']
        for k in diff_keys: features[k] = np.nan
...
def extract_features_with_mask(
    ...
    if isinstance(frames_or_path, str):
        ...
        try: mat_key = getattr(config, 'MAT_FRAMES_KEY', 'TempFrames'); mat_data = scipy.io.loadmat(frames_or_path); frames = mat_data.get(mat_key, None); assert frames is not None; frames = frames.astype(np.float64)
    ...
    # --- Call the core function, passing the fixed threshold ---
...
# --- END OF MODIFIED feature_engineering.py ---