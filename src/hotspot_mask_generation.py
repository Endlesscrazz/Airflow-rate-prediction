# --- START OF FILE hotspot_mask_generation.py (P-Value Filtered Slope ONLY) ---
...
def apply_preprocessing(frames, normalizeT, fuselevel):
    ...
    if normalizeT:
        frame_means = np.mean(frames_proc, axis=(0, 1), keepdims=True); frame_means[frame_means == 0] = 1.0; frames_proc /= frame_means
    if fuselevel > 0:
        kernel_size = 2 * fuselevel + 1
        try:
            num_frames = frames_proc.shape[2]; frames_fused = np.zeros_like(frames_proc)
            for i in range(num_frames): frames_fused[:, :, i] = cv2.boxFilter(frames_proc[:, :, i], -1, (kernel_size, kernel_size))
            frames_proc = frames_fused
        except Exception as e: print(f"Warning: Spatial fuse failed: {e}")
    return frames_proc

# --- calculate_activity_map_from_difference REMOVED ---
...
def calculate_filtered_slope_activity_map(frames_focus, fps, smooth_window,
    ...
    if smooth_window > 1:
        ...
        for r in range(H):
            for c in range(W):
                smoothed_series = np.convolve(frames_focus[r, c, :], np.ones(smooth_window)/smooth_window, mode='valid')
                pad_len_before = (T_focus - len(smoothed_series)) // 2
                pad_len_after = T_focus - len(smoothed_series) - pad_len_before
                if len(smoothed_series)>0: frames_focus_smoothed[r,c,:] = np.pad(smoothed_series, (pad_len_before, pad_len_after), mode='edge')
                else: frames_focus_smoothed[r,c,:] = np.nan
        t_smoothed = (np.arange(T_focus) / fps).astype(np.float64)
    else:
        frames_focus_smoothed = frames_focus
        t_smoothed = (np.arange(T_focus) / fps).astype(np.float64)

    if T_focus < 2: print("Error: Less than 2 time points."); return None

    # Determine iteration indices
    if roi_mask is not None:
        roi_indices = np.argwhere(roi_mask)
        iterator = tqdm(range(roi_indices.shape[0]), desc="Calculating Filtered Slope (ROI)", leave=False, ncols=80)
        get_coords = lambda idx: roi_indices[idx]
    else:
        total_pixels = H * W
        iterator = tqdm(range(total_pixels), desc="Calculating Filtered Slope (Full)", leave=False, ncols=80)
        get_coords = lambda idx: np.unravel_index(idx, (H, W))

    for idx in iterator:
        r, c = get_coords(idx)
        y = frames_focus_smoothed[r, c, :]
        valid_mask_y = np.isfinite(y); y_valid = y[valid_mask_y]; t_valid = t_smoothed[valid_mask_y]
        if len(y_valid) < 2: continue
        try:
            ...
            if passes_filter:
                ...
                if augment != 1.0: activity_value = np.power(activity_value, augment)
                final_activity_map[r, c] = activity_value
        except ValueError: pass
        except Exception as e_lr: print(f"\nWarn: linregress/filter failed at ({r},{c}): {e_lr}"); pass
...
def extract_hotspot_from_map(activity_map, threshold_quantile=0.95, morphology_op='close',
                             ...
                             roi_mask=None):
    # (Keep function as before - ROI application is correct here)
    # ... (exact same implementation as in your uploaded hotspot_mask_generation.py) ...
    ...
    try:
        ...
        elif morphology_op != 'none': print(f"Warning: Unknown morph_op '{morphology_op}'. Using 'none'.");
    except cv2.error as e: print(f"Warning: Morphology error: {e}. Using raw binary mask."); mask_processed = binary_mask
    try: num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(mask_processed, connectivity=8)
    ...
    if num_labels > 1 and stats.shape[0] > 1:
        ...
        if foreground_stats.size > 0: largest_label_idx = np.argmax(foreground_stats[:, cv2.CC_STAT_AREA]); largest_label = largest_label_idx + 1; hotspot_mask = (labels == largest_label); hotspot_area = np.sum(hotspot_mask)
...
def generate_masks_for_dataset(dataset_root_dir, output_base_dir, mat_key, fps, focus_duration_sec,
    ...
    print(f"  Focus Duration: {focus_duration_sec}s | FPS: {fps} | Smooth: {smooth_window}")
    print(f"  P-Value Thresh: {p_value_threshold} | Quantile: {quantile:.3f} | Morphology: {morphology_op}")
    print(f"  Blur: {apply_blur} | Fuse Level: {fuselevel} | NormalizeT: {normalizeT}")
    print(f"  Envir: {envir_para} | Augment: {augment}")
    if roi_border_percent is not None: print(f"  ROI Border: {roi_border_percent*100:.0f}%")
    else: print("  ROI Border: None (slope on full, extraction uses ROI if defined)")
    print("-" * 30)
    ...
    for root, _, files in os.walk(dataset_root_dir):
        for mat_file in fnmatch.filter(files, '*.mat'):
            ...
            # Load Data
            try:
                mat_data=scipy.io.loadmat(mat_file_path); frames_raw=mat_data[mat_key].astype(np.float64)
                if frames_raw.ndim!=3 or frames_raw.shape[2]<2: raise ValueError("Invalid frame data.")
                H, W, num_frames = frames_raw.shape
            except Exception as e: print(f"  Error loading data: {e}. Skip."); error_count+=1; continue

            # Pre-processing
            frames_proc = apply_preprocessing(frames_raw, normalizeT, fuselevel)
            # Focus Window
            focus_duration_frames = int(focus_duration_sec * fps); focus_duration_frames = max(2, min(focus_duration_frames, num_frames))
            frames_focus = frames_proc[:, :, :focus_duration_frames]

            # --- Define ROI Mask (for slope calculation if desired, and for extraction) ---
            ...
            if roi_border_percent is not None:
                ...
                roi_mask_for_slope_calc[border_h:-border_h, border_w:-border_w]=True
            ...
            if activity_map is None or not np.any(np.isfinite(activity_map)): print("Error: Failed valid activity map generation."); error_count+=1; continue

            # --- Extract Hotspot Mask ---
            # ROI mask for extraction is the same one (roi_mask_for_slope_calc)
            hotspot_mask, hotspot_area = extract_hotspot_from_map(
                activity_map, quantile, morphology_op, apply_blur, tuple(blur_kernel_size), roi_mask_for_slope_calc
            )

            if hotspot_mask is None: print("Error: Failed mask extraction."); error_count+=1; continue
            elif hotspot_area == 0: print("Warning: Zero area mask generated.")
            ...
            try:
                ...
                processed_count += 1
            except Exception as e: print(f"Error saving mask: {e}"); error_count += 1

    print("-" * 30); print(f"Mask generation finished. Success: {processed_count}, Errors: {error_count}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-k", "--key", default=config.MAT_FRAMES_KEY, help=f"Key in .mat file (default from config).")
    parser.add_argument("--fps", type=float, default=5.0, help="Frames per second of original recording.")

    # Slope Analysis Params
    parser.add_argument("--focus_duration_sec", type=float, default=10.0, help="Duration (s) for slope analysis.")
    parser.add_argument("--smooth_window", type=int, default=1, help="Temporal smoothing window for slope fit (1=None).")
    parser.add_argument("--p_value_thresh", type=float, default=0.10, help="P-value threshold for slope significance filter.")
    parser.add_argument("--envir_para", type=int, default=-1, choices=[-1, 1], help="Environment parameter (-1=Winter, 1=Summer).")
    parser.add_argument("--augment", type=float, default=1.0, help="Exponent for augmenting filtered slope magnitude.")

    # Pre-processing Args
    parser.add_argument("--normalizeT", type=int, default=0, choices=[0, 1], help="Normalize temperature per frame? (0=No, 1=Yes).")
    parser.add_argument("--fuselevel", type=int, default=0, help="Spatial fuse level (0=None).")
    ...
    parser.add_argument("-q", "--quantile", type=float, default=0.99, help="Quantile threshold (0-1).")
    parser.add_argument("--morph_op", default="none", choices=['close', 'open_close', 'none'], help="Morphological operation.")
    parser.add_argument("--blur", action='store_true', help="Apply Gaussian blur to activity map.")
    parser.add_argument("--blur_kernel", type=int, nargs=2, default=[3, 3], metavar=('H', 'W'), help="Kernel size for Gaussian blur.")
    # Removed --method and --diff_time_sec as we reverted to slope-only

    if len(sys.argv) < 2: parser.print_help(); sys.exit(1)
    args = parser.parse_args()

    # --- Argument Validation ---
    # (Keep relevant validations)
    if not 0.0 < args.quantile <= 1.0: print("Error: Quantile invalid."); sys.exit(1)
    if args.focus_duration_sec <= 0: print("Error: Focus duration invalid."); sys.exit(1)
    if args.fps <= 0: print("Error: FPS invalid."); sys.exit(1)
    if args.blur: kh, kw = args.blur_kernel; assert kh > 0 and kw > 0; assert kh % 2 != 0 and kw % 2 != 0, "Blur kernel dims must be odd"
    # ROI default is now 0.20, so None check is different.
    # If you want to disable ROI, user should pass a very large border percent or we add a specific flag.
    # Let's assume 0.20 is a sensible default for now, can be set to None via config if config.ROI_BORDER_PERCENT = None
    if args.roi_border_percent is not None and not 0.0 <= args.roi_border_percent < 0.5:
         print("Error: ROI border percent must be >= 0 and < 0.5 if specified."); sys.exit(1)

    if args.fuselevel < 0: print("Error: Fuse level invalid."); sys.exit(1)
    if args.smooth_window < 1: print("Error: Smooth window invalid."); sys.exit(1)
    if not 0.0 < args.p_value_thresh < 1.0: print("Error: P-value threshold invalid."); sys.exit(1)
    if args.augment <= 0: print("Error: Augment invalid."); sys.exit(1)
    ...
    # Set Output Dir
    output_mask_dir = args.output_dir; os.makedirs(output_mask_dir, exist_ok=True)
...
# --- END OF FILE hotspot_mask_generation.py ---