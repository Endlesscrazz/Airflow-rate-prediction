...
3. Runs GridSearchCV with GroupKFold for multiple model types to ensure
   no data leakage during hyperparameter tuning.
...
def augment_features(X_df, y_series, groups_series, num_copies=1, noise_level=0.03):
    """
    Augments features by adding Gaussian noise. Correctly handles groups.
    """
    if num_copies == 0:
        return X_df, y_series, groups_series

    augmented_X_list, augmented_y_list, augmented_groups_list = [], [], []
    # Ensure groups_series is not None before trying to use it
    if groups_series is None:
        raise ValueError("groups_series cannot be None for augmentation.")

    numeric_cols = [
        col for col in X_df.columns if not col.startswith('material_')]

    for _ in range(num_copies):
        X_aug = X_df.copy()
        for col in numeric_cols:
            if X_df[col].std() > 1e-6:  # Avoid adding noise to constant columns
                noise = np.random.normal(
                    0, noise_level, size=len(X_df)) * X_df[col].std()
                X_aug[col] += noise
        augmented_X_list.append(X_aug)
        augmented_y_list.append(y_series)
        # Append the original groups for the new copies
    ...
    groups_final = pd.concat(
        [groups_series] + augmented_groups_list, ignore_index=True)
...
def main():
    ...
    if not os.path.exists(train_path):
        sys.exit(
            f"FATAL ERROR: {train_path} not found. Run generate_features.py and split_data.py first.")

    train_df = pd.read_csv(train_path)

    y_dev_original = train_df['airflow_rate']

    train_df['base_video_id'] = train_df['video_id'].apply(
        lambda x: x.split('_hole_')[0])
    groups_dev = train_df['base_video_id']

    # Drop identifiers not used as features
    X_raw = train_df.drop(
        columns=['airflow_rate', 'video_id', 'hole_id', 'base_video_id'])
    ...
    # Define raw columns to exclude from the final feature set
    ...
    X_transformed = pd.concat([X_transformed, pd.get_dummies(
        X_raw['material'], prefix='material', dtype=int)], axis=1)
    # Session is NOT used as a training feature in this experimental setup

    print("\nCreating interaction features (if any)...")
    # (Interaction logic is fine, but can be commented out if not using interactions)
    ...
    # --- CORRECT CV SPLITTER FOR THIS STRATEGY ---
    ...
    for model_name, model in models.items():
        ...
        best_mae_log = - \
            results['mean_test_neg_mean_absolute_error'][best_index]
        best_rmse_log = - \
            results['mean_test_neg_root_mean_squared_error'][best_index]
    ...
    # --- CORRECT PLOTTING CALL WITH GROUPS ---
    ...
    fi_path = os.path.join(
        plots_dir, f"feature_importance_{best_model_name}.png")
