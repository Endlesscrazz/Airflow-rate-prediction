...
def plot_residuals_vs_fitted(y_true, y_pred, material_labels, save_path):
    """Plots residuals vs. fitted values, separated by material."""
    residuals = y_true - y_pred
    fig, ax = plt.subplots(figsize=(10, 7))
    
    materials = np.unique(material_labels)
    colors = plt.cm.viridis(np.linspace(0, 1, len(materials)))
    
    for i, material in enumerate(materials):
        mask = (material_labels == material)
        ax.scatter(y_pred[mask], residuals[mask], alpha=0.6, color=colors[i], label=material)
...
def plot_permutation_importance(model, X, y, feature_names, save_path):
    ...
    ax.set_xlabel("Importance Score (Decrease in R²)")
    fig.tight_layout()
    fig.savefig(save_path)
    plt.close(fig)

def plot_per_fold_predictions(y_true, y_pred, fold_indices, save_path):
    """Plots actual vs. predicted, colored by outer fold index."""
    fig, ax = plt.subplots(figsize=(10, 10))
    
    unique_folds = np.unique(fold_indices)
    colors = plt.cm.viridis(np.linspace(0, 1, len(unique_folds)))
    
    for i, fold_idx in enumerate(unique_folds):
        mask = (fold_indices == fold_idx)
        ax.scatter(y_true[mask], y_pred[mask], color=colors[i], label=f'Fold {fold_idx}', alpha=0.7)
        
    lims = [np.min([ax.get_xlim(), ax.get_ylim()]), np.max([ax.get_xlim(), ax.get_ylim()])]
    ax.plot(lims, lims, 'r--', alpha=0.75, zorder=0, label='Ideal (y=x)')
    ax.set_xlim(lims); ax.set_ylim(lims)
    ax.set_xlabel("Actual Airflow Rate")
    ax.set_ylabel("Predicted Airflow Rate")
    ax.set_title("Per-Fold Actual vs. Predicted (Nested CV)")
    ax.legend()
    ax.grid(True)
    fig.savefig(save_path)
    plt.close(fig)