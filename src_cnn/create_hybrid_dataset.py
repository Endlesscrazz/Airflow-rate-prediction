...
# --- Configuration ---
CNN_DATASET_DIR = "cnn_dataset/dataset_cnn_lstm_3-channel"
METADATA_SAVE_PATH = os.path.join(CNN_DATASET_DIR, "metadata.csv")
DATASET_TYPE = "3-Channel Hybrid (Thermal + Flow)"
    ...
    for sample_info in tqdm(all_samples_info_list, desc="Extracting Sequences"):
        try:
            ...
            for i, individual_mask in enumerate(masks_to_process):
                ...
                processed_hybrid_sequence = []
                for frame_idx in range(NUM_FRAMES_PER_SAMPLE - 1):
                    # Get two consecutive ROIs for flow calculation
                    ...
                    # --- 1. Get the THERMAL channel ---
                    # Normalize the first frame of the pair to [0, 1] to be used as the thermal channel
                    min_val, max_val = prev_roi_resized.min(), prev_roi_resized.max()
                    if max_val > min_val:
                        thermal_channel = (prev_roi_resized - min_val) / (max_val - min_val)
                    else:
                        thermal_channel = np.zeros_like(prev_roi_resized)
                    
                    # --- 2. Get the FLOW channels ---
                    ...
                    # --- 3. Stack all 3 channels together ---
                    # flow[:, :, 0] is the x-flow
                    # flow[:, :, 1] is the y-flow
                    # The 'dstack' function stacks arrays along the third dimension (depth).
                    hybrid_frame = np.dstack([thermal_channel, flow[:, :, 0], flow[:, :, 1]])
                    
                    processed_hybrid_sequence.append(hybrid_frame)

                # Stack the sequence of 3-channel frames
                # Resulting shape: (24, 128, 128, 3)
                final_hybrid_array = np.stack(processed_hybrid_sequence, axis=0)
                ...
                np.save(image_save_path_full, final_hybrid_array)
...
# python -m src_cnn.create_hybrid_dataset