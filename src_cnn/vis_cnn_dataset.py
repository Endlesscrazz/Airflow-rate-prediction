...
import argparse
import cv2 # Needed for optical flow visualization
...
def visualize_dataset(dataset_dir, max_samples, output_dir_name):
    ...
    VISUALIZATION_OUTPUT_DIR = os.path.join("verified_masks/visualizations_cnn", output_dir_name)
    ...
    if not os.path.isfile(PARAMS_PATH):
        print(f"WARNING: dataset_parameters.txt not found. Cannot determine dataset type automatically.", file=sys.stderr)
        # Attempt to infer from a sample file
        try:
            ...
            print(f"Inferred {num_channels} channels from sample file.")
        except Exception:
            print("Could not infer channel count. Please check your dataset.")
            sys.exit(1)
    else:
        # Read the number of channels from the parameters file
    ...
    # 3. Load and Sample Metadata
    df_metadata = pd.read_csv(METADATA_PATH)
    if max_samples is not None and max_samples < len(df_metadata):
        samples_to_process = df_metadata.sample(n=max_samples, random_state=42)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_dir", type=str, required=True,
                        help="Path to the root directory of the dataset to visualize (e.g., 'cnn_dataset/dataset_2ch_flow').")
    ...
    output_dir_name = os.path.basename(args.dataset_dir) # Use dataset name for output folder
